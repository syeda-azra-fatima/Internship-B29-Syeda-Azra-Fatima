...
```{r}
preds_train_knn<-predict(model_knn,train_data2,type = "prob")
#prediction(preds_train_knn,train_data2)
preds_test_knn <- predict(model_knn,test_data2, type = "prob")
```
...
library(ROCR)
library(pROC)
pred1<- prediction(model_knn)
perform_knn = performance(preds_test_knn, measure="tpr", x.measure="fpr")
